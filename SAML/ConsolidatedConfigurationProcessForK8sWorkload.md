...
# Doxis CSB + webCube Passwordless Login via SAML (Okta) for [Doxis provided webCube Helm chart](https://services.sergroup.com/documentation/#/view/helm_doxis_webcube/30.0.1/en-us/DG_HELM/WEBHELP/index.html)
...
5. **Kubernetes/Helm (if applicable)**

   * Used to configure webCube deployment settings, including **CSP rules**
...
| `<OKTA_SSO_HOST>`         | Okta SSO hostname users are redirected to (e.g., `applogin.example.com`) |
...
## Step 7 — Ensure webCube CSP allows posting the SAML request to the IdP

**Component: Doxis webCube (Deployment / Helm / Web Server Headers)**

webCube initiates SAML by returning an HTML form that auto-submits (`POST`) to the IdP SSO URL.
If the webCube **Content-Security-Policy** blocks cross-domain form posting, the browser will block
the POST and users will see a blank/white page.

### If using the webCube Helm chart

Add/extend CSP rules via:

* `webcube.contentSecurityPolicy.customRules`

Example (allow the IdP SSO host used by your environment):
...
webcube:
  contentSecurityPolicy:
    mode: "PRODUCTION"
    customRules:
      - "form-action 'self' https://<OKTA_SSO_HOST> *.office.com *.officeapps.live.com *.microsoftonline.com *.mi;"
```

### Verification

After deployment, confirm the `Content-Security-Policy` response header includes the IdP host in `form-action`
(e.g., via browser DevTools or `curl -I`), and that the browser is able to POST to the IdP endpoint.

---

## Step 8 — Enable SAML in webCube
...
## Step 9 — Enable SAML for CSB Users
...
## Step 10 — End-user Login Flow (Passwordless)
...
## Swimlane of configuration process (updated)
...
sequenceDiagram
  ...
  participant HELM as Helm/K8s (Deployment)
  ...
  %% --- webCube CSP allowlist (new) ---
  rect rgb(240,240,240)
    note over HELM,WC: [G7] webCube CSP allowlist
    HELM->>HELM: Add CSP customRules\n(form-action allows IdP SSO host)
    HELM-->>WC: Deploy/restart webCube with updated CSP
  end

  %% --- webCube enablement ---
  rect rgb(240,240,240)
    note over WC: [G8] webCube enablement
    WC->>WC: Create / edit server profile
    WC->>WC: Enable SAML authentication
  end

  %% --- User enablement ---
  note over CSB: [G9] Enable SAML authentication\nfor users (cubeDesigner)
```

## Swimlane / Process Flow Diagram (End to End) (updated)
...
sequenceDiagram
  ...
  rect rgb(240,240,240)
    note over HELM,WC: Ensure CSP allows IdP POST
    HELM->>HELM: Set contentSecurityPolicy.customRules\n(form-action includes IdP SSO host)
    HELM-->>WC: Deploy/restart webCube
  end

  rect rgb(230,245,255)
    U->>WC: Open webCube URL
    U->>WC: Click "Log on with SAML"
    WC-->>U: HTML form (auto-submit)\nPOST to IdP SSO endpoint
    U->>OKTA: POST AuthnRequest (allowed by CSP)
